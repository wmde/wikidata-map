 ...
 */
self.startFetch = async function( layerKey, dateStr, intensityScale, propertyLayerConfig ) {
	console.log("Worker: Rendering canvas " + dateStr + " " + layerKey)
	if(layerKey.lastIndexOf("items", 0) === 0) {
		let mapUrl = 'https://analytics.wikimedia.org/published/datasets/one-off/wikidata/wmde_wikidata_map/map-' + dateStr + '-7680-4320-pixels.csv'
		...
		propertyLayerConfig.forEach( function(layer){
			...
			if(propertyId == layerKey) {
				let layerUrl = 'https://analytics.wikimedia.org/published/datasets/one-off/wikidata/wmde_wikidata_map/map-' + dateStr + '-7680-4320-relation-pixels-{property}.csv'
...
onmessage = function(e) {
	const [layerKey, dateStr, intensityScale, propertyLayerConfig] = e.data;
	console.log('Worker: Message to render canvas ' + dateStr + " " + layerKey)
	self.startFetch( layerKey, dateStr, intensityScale, propertyLayerConfig )
}
